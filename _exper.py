...
class WulffShape(object):
    ...
    def __init__(self, lattice, miller_list, e_surf_list, symprec=1e-5):
        ...
        for m, in_mill_fig in enumerate(self.input_miller_fig):
            miller_area.append(
                in_mill_fig + ' : ' + str(round(self.color_area[m], 4))) #rounded to 4 initially
    ...
    def get_plot(self, color_set='PuBu', grid_off=True, axis_off=True,
        ...
        if legend_on:
            ...
            if show_area:
                #print ('----------------------------------')
                #print (color_proxy)
                #print (self.miller_area)
                sma=self.miller_area
                import _experhelp
                
                swig,sorti1,sorti2=_experhelp.sorter(sma)
                cp=np.array(color_proxy,dtype=object)
                ecp=cp[sorti1]
                ecp=ecp[sorti2]
                #ax.set_pane_color([0.5,0.5,0.5,1])
                #ax.facecolor='gray')
                ax.legend(ecp, swig, loc='upper left',
        ...
        def cleanup(bounds,r):
            #take bounds and remove ticks too close
            nbounds=[]
            ntickmarks=[]
            if len(bounds)==1:
                endb=bounds[-1]*1.2
            else:
                avg=(bounds[-1]-bounds[0])/float(len(bounds)-1)
                endb=bounds[-1]+avg
            span=endb-bounds[0]
            #print (span,endb,bounds)
            bf=bounds[0]
            for b in bounds:
                rb=round(b,r)
                if b == bf:
                    nbounds.append(rb)
                    ntickmarks.append(rb)
                    bf=b
                else:
                    closen=(b-bf)/span
                    if closen > 0.08:
                        #print(b,bf,closen,span)
                        nbounds.append(rb)
                        ntickmarks.append(rb)
                        bf=b
                    else:
        ...
        if bar_on:
            ...
            cmap.set_under('0.75')
            '''
            e_surf_on_wulff=np.array(e_surf_on_wulff)
            print (e_surf_on_wulff)
            e_surf_on_wulff=e_surf_on_wulff[sorti1]
            e_surf_on_wulff=e_surf_on_wulff[sorti2]
            '''
            if min(e_surf_on_wulff) < 1: #if it is a reaxff wulff
                r=6
                bounds=[round(e, r) for e in e_surf_on_wulff]
                tr=2
                tbounds=[round(e, tr) for e in e_surf_on_wulff]
                if max(tbounds) == min(tbounds) and len(tbounds) != 1 :
                    r=3
                else:
                    r=3
                tickmarks,bounds=cleanup(bounds,r)
            else: #if it is estimate
                r=6
                bounds=[round(e, r) for e in e_surf_on_wulff]
                tr=2
                tbounds=[round(e, tr) for e in e_surf_on_wulff]
            ...
            #figure out where to put the bounds	
            if len(bounds)==1:
                bounds.append(bounds[-1]*1.2)
                tickmarks.append(bounds[-1]*1.2)
                #print(bounds, 'bounds baron')
            else:
                avg=(bounds[-1]-bounds[0])/float(len(bounds)-1)
                endb=bounds[-1]+avg
                #print (avg,endb)
                bounds.append(endb)
                tickmarks.append(endb)
                bounds[0]=tickmarks[0]
                #print(bounds, 'bounds baron')
            #print (bounds, '4')
            #print (tickmarks, '4')
            tickmarks=tickmarks[0:-1]
            '''
            nbounds=[]
            for b in bounds:
                nb=b*100
            ...
            cbar = mpl.colorbar.ColorbarBase(
                ax1, cmap=cmap, norm=norm, boundaries=[min(bounds)-0.00000000001] + bounds + [max(bounds)],
                extend='both', extendfrac=0.000001, ticks=tickmarks, spacing='proportional', #pacing uniform vs proportional
                orientation='horizontal')
            #cbar = mpl.colorbar.ColorbarBase(
            #    ax1, cmap=cmap, norm=norm, boundaries=[0] + bounds + [10],
            #    extend='both', ticks=bounds[0:-1], spacing='proportional',
            #    orientation='vertical')
            cbar.set_label('Surface Energies ($Broken Bonds/Å^2$)', fontsize=1)
